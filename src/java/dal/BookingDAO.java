...
public class BookingDAO extends DBContext {
    ...
    /**
     * Get complete booking details with movie, screening, tickets, and food
     * information
     */
    ...
    /**
     * Parse Showtime string (e.g., "08:00-10:00") and combine with
     * ScreeningDate to create LocalDateTime objects for StartTime and EndTime
     */
    ...
    // Khách hàng yêu cầu hoàn tiền -> Chuyển status thành "RefundRequested"
    public boolean requestRefund(int bookingID) {
        String sql = "UPDATE Bookings SET Status = 'RefundRequested' WHERE BookingID = ?";
        try (Connection conn = getConnection(); PreparedStatement ps = conn.prepareStatement(sql)) {
            ps.setInt(1, bookingID);
            return ps.executeUpdate() > 0;
        } catch (SQLException e) {
            System.err.println("Error requesting refund: " + e.getMessage());
            e.printStackTrace();
            return false;
        }
    }

    // Staff lấy tất cả các đơn đang chờ hoàn tiền
    public List<BookingDetailDTO> getRefundRequests() {
        List<BookingDetailDTO> bookingDetails = new ArrayList<>();
        String sql = "SELECT b.BookingID, b.BookingCode, b.BookingDate, b.Status, u.FullName "
                + "FROM Bookings b "
        ...
        try (Connection conn = getConnection(); PreparedStatement ps = conn.prepareStatement(sql)) {
            ...
            System.err.println("Error getting refund requests: " + e.getMessage());
        }
        return bookingDetails;
    }
...
public boolean approveRefund(int bookingID) {
    ...
    // Staff từ chối hoàn tiền -> Chuyển status về Confirmed
    public boolean denyRefund(int bookingID) {
        String sql = "UPDATE Bookings SET Status = 'Confirmed' WHERE BookingID = ? AND Status = 'RefundRequested'";
        try (Connection conn = getConnection(); PreparedStatement ps = conn.prepareStatement(sql)) {
            ps.setInt(1, bookingID);
            return ps.executeUpdate() > 0;
        } catch (SQLException e) {
            System.err.println("Error denying refund: " + e.getMessage());
            return false;
        }
    }
