   ...
   public void insertScreening(Screening sc) {
    ...
    public Screening getScreeningDetail(int screeningID) {
        ...
        String sql = "SELECT s.ScreeningID, s.MovieID, s.RoomID, "
                   ...
                   + "ISNULL((SELECT COUNT(*) FROM Seats WHERE RoomID = s.RoomID AND Status NOT IN ('Unavailable', 'Maintenance')), 0) "
                   + "- ISNULL((SELECT COUNT(DISTINCT t.SeatID) "
                   + "FROM Tickets t INNER JOIN Bookings b ON t.BookingID = b.BookingID "
                   + "WHERE t.ScreeningID = s.ScreeningID AND b.Status IN ('Confirmed', 'Completed')), 0) "
