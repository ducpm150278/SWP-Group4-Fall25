...
public class GuestMovieListServlet extends HttpServlet {
    ...
    throws ServletException, IOException {
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet GuestMovieListServlet</title>");  
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet GuestMovieListServlet at " + request.getContextPath () + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    } 
    ...
    throws ServletException, IOException {
        ...
        int page = 1;
        int recordsPerPage = 12; // Show 12 movies per page as specified
        ...
        // Filter movies by status and keyword
        if (status != null && !status.isEmpty()) {
            movies = dao.getMoviesByStatus(status, keyword);
            totalRecords = movies.size();
        } else {
            // Get all active movies (both Active and Upcoming)
        ...
        request.getRequestDispatcher("guest-movies.jsp").forward(request, response);
    } 
