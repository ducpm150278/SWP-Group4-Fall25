...
public class ListScreeningServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        // Có điều kiện lọc/tìm kiếm
        if ((keyword != null && !keyword.isEmpty())
                || from != null || to != null
                || (showtime != null && !showtime.isEmpty())
                || (status != null && !status.isEmpty())) {

            list = dao.searchScreeningsWithPaging(keyword, from, to, showtime, status, page, pageSize);
            totalRecords = dao.countSearchScreenings(keyword, from, to, showtime, status);
        ...
        request.getRequestDispatcher(Constants.JSP_SCREENING_LIST).forward(request, response);
    }
